...
def run_multi_day_compounding():
    ...
    while current_dt < end_dt:
        ...
        chunk = yf.download(symbol, start=current_dt.strftime('%Y-%m-%d'), end=chunk_end.strftime('%Y-%m-%d'), interval='1m', progress=False)
        if not chunk.empty:
    ...
    for date in unique_dates:
        ...
        tester = banknifty_backtest.Backtester(daily_data, starting_capital=current_capital)
        tester.run()
