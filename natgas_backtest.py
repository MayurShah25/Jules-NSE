...
SLIPPAGE = 0.1 # fixed slippage in points
MAX_CAPITAL_DEPLOYMENT = 0.95 # Use 95% of available capital per trade

# Realistic MCX Fees (Approximate)
BROKERAGE_PER_ORDER = 20.0
STT_PCT = 0.0001 # 0.01% on sell side premium for futures
EXCHANGE_TXN_CHARGE_PCT = 0.000021 # MCX txn charge
GST_PCT = 0.18 # 18% on (brokerage + txn charge)
SEBI_CHARGE_PCT = 0.000001 # Rs 10 per crore
STAMP_DUTY_PCT = 0.00002 # 0.002% on buy side premium for futures
...
class Backtester:
    ...
    def run(self):
        ...
        for idx, row in df.iterrows():
            ...
            # Intraday Market Hours (MCX 09:00 to 23:15)
            # Brokers enforce MIS square-off around 11:15 PM. We use 23:15 to prevent overnight holds.
            if current_time >= time(23, 15):
                if self.in_position:
                    current_fut_price = row['close']
                    self._exit_trade(row, current_fut_price, "MIS Auto Square Off (23:15)")
                continue
...
if __name__ == "__main__":
    logger.info("Running backtest on actual yfinance data...")
    try:
        df = pd.read_csv('natgas_data.csv', parse_dates=['Datetime'], index_col='Datetime')
        # Use localized time for correct market hour filtering
        df.index = df.index.tz_convert('Asia/Kolkata')

        # Optional: Filter for MCX standard hours 9:00 AM to 11:30 PM
        df = df.between_time('09:00', '23:30')

        tester = Backtester(df)
        tester.run()
    except FileNotFoundError:
        logger.error("natgas_data.csv not found. Skipped yfinance mock data for now.")
