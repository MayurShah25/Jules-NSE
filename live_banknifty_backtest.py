...
# Fetch 5 days of Bank BankNifty data to "warm up" the indicators
# (e.g. 21-EMA and 30-min rolling breakouts need prior data if run right at market open)
print("Fetching live BankNifty data to warm up indicators...")
nifty = yf.download('^NSEBANK', period='5d', interval='1m', progress=False)
...
if nifty.empty:
    ...
    # We might not have volume for Bank Nifty Index directly, so mock it if missing or 0
    ...
    if today_candles < 2:
        ...
        tester = banknifty_backtest.Backtester(nifty)
